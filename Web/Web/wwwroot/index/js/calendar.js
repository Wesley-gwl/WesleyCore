...
Calendar.prototype = {
    ...
    _refreshYearCalendar: function (newDate) {
        ...
        var dateNow = me._cloneDate(new Date());
                ...
                if (showEvent && viewData[renderMonth]) {
                    if (maxEvent && viewData[renderMonth].length > maxEvent) {
                        ...
                        viewData[renderMonth].forEach(function (item) {
                            if (me._getDateByString(item.startDate) <= dateNow) {
                                s += '<li><span style="color:red">' + item.name + '</span></li>'
                            }
                            else {
                                s += '<li><span style="color:green">' + item.name + '</span></li>'
                            }
                        })
                    }
                }
    ...
    _refreshCalendar: function (newDate) {
        ...
        for (var i = 0; i < 6; i++) {
            s += '<tr>'
            for (var l = 0; l < 7; l++) {
                var year = renderDate.getFullYear();//年
                var month = renderDate.getMonth() + 1;//月
                var date = renderDate.getDate();//日
