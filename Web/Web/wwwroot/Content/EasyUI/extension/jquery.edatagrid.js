...
(function ($) {
    ...
    function buildGrid(target) {
        ...
        // 点击空白处将会结束（取消）编辑
        $(target).datagrid('getPanel').click(function (e) {
            var td = $(e.target).closest('td[field]');
            if (!td.length) {
                $(target).datagrid('endEdit', opts.editIndex);
                var isValid = $(target).datagrid('validateRow', opts.editIndex);
                var opera = isValid ? 'endEdit' : 'cancelEdit';
                $(target).datagrid(opera, opts.editIndex);
            }
        })
    ...
    $.fn.edatagrid.methods = {
        ...
        editRow: function (jq, index) {
            return jq.each(function () {
                var dg = $(this);
                var opts = $.data(this, 'edatagrid').options;
                // 针对在编辑状态下关闭dialog的情况（editIndex不会清空，那么第二次打开就会出现下述情况）
                // 1. 存在 editIndex 编辑行
                // 2. 但没有正在编辑的行（编辑器）
                var editors = dg.datagrid('getEditors', opts.editIndex);
                var editIndex = editors.length ? opts.editIndex : -1;
                if (editIndex != index) {
