...
(function ($) {
    $.extend($.fn.datagrid.defaults, {
        ...
        onBeforeLoad: function (param) {
            if (!$(this).datagrid('options').datagridColumnColor || $(this).datagrid('options').datagridColumnColor.length < 1)
                return;
            //设置列的字体颜色
            $(this).datagrid('options').datagridColumnColor.forEach((color) => {
                var dg = this.parentElement;
                if (color.field)
                    color.field.forEach((f) => {
                        //header
                        $(dg).find(".datagrid-header-row [field='" + f + "']").each((i, c) => {
                            c.style = "color:" + color.color
                        });
                    });
            });
        }
